...
class _MarcarConsultaPageState extends State<MarcarConsultaPage> {
  ...
  Future<void> _carregarHorariosDisponiveis() async {
    if (_especialidadeSelecionada == null || _diaSelecionado == null || _localSelecionado == null) return;
    ...
    try {
      ...
      final horariosOcupados = consultasDoDia.docs.map((doc) => doc['hora'] as String).toList();

      if (!mounted) return;
      setState(() {
        horariosDisponiveis = todosHorarios.where((hora) => !horariosOcupados.contains(hora)).toList();
        _horaController.clear();
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _marcarConsulta() async {
    ...
    try {
      await FirebaseFirestore.instance.collection('Consultas').add(novaConsulta.toMap());
          ...
          child: ListView(
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: locais
                    .map((local) => DropdownMenuItem(value: local, child: Text(local)))
                    .toList(),
              ...
              if (diasDisponiveis.isNotEmpty) ...[
                ...
                TableCalendar(
                  ...
                  onDaySelected: (selectedDay, _) async {
                    final selecionado = DateFormat('yyyy-MM-dd').format(selectedDay);
                  ...
                  calendarBuilders: CalendarBuilders(
                    defaultBuilder: (context, day, _) {
                      ...
                      final isDisponivel = diasDisponiveis.contains(dataStr);
                      return Center(
                        child: Text(
                          '${day.day}',
                          style: TextStyle(
                            color: isDisponivel ? Colors.black : Colors.grey,
                          ),
                        ),
                      );
                    },
